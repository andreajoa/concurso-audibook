...
module.exports=async(req,res)=>{
  ...
  try{const result=await rpc('crm_newsletter_signup',{payload:{email,consent:true,source:'storefront'}});if(result?.ok!==true)throw new Error('signup_failed');return res.status(200).json({ok:true});}
