...
function validateProductAssets(product) {
  if (!product?.slug || !product?.assets) throw new Error('Invalid product.');
  ...
  if (keys.length !== 11 || new Set(keys).size !== 11 || keys.some(key =>
    typeof key !== 'string' || !key.startsWith(`${product.slug}/`) || key.includes('..'))) {
    throw new Error('Product assets must belong exclusively to the purchased product.');
  }
}

module.exports = { signObject, signedProductAssets, validateProductAssets };
