...
/**
 * O banco já sabe guardar caderno?
 *
 * Existe uma janela real em que o código está publicado e a migration ainda
 * não foi aplicada. Sem esta checagem, essa janela cobra de alguém por uma
 * sincronização que não tem onde acontecer: o pagamento passa, o webhook falha
 * ao registrar e a pessoa fica com uma cobrança e nenhuma chave. Não vender é
 * a única saída honesta, e é barata — a resposta positiva fica em memória para
 * sempre, e a negativa por um minuto, para o site voltar a vender sozinho
 * assim que a migration rodar.
 */
let prontidao = { valor: null, ate: 0 };
async function bancoPronto() {
  if (prontidao.valor === true) return true;
  if (prontidao.valor === false && Date.now() < prontidao.ate) return false;
  try {
    // Uma chave que não existe: a resposta não importa, só o fato de a função existir.
    await rpc('caderno_puxar', { payload: { chave: '0'.repeat(48) } });
    prontidao = { valor: true, ate: 0 };
  } catch (error) {
    console.error('caderno_banco_indisponivel', error.message);
    prontidao = { valor: false, ate: Date.now() + 60000 };
  }
  return prontidao.valor;
}
...
module.exports = {
  ...
  registrarAssinante, atualizarStatus, chaveDaSessao, bancoPronto
};
