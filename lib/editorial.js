...
const FORBIDDEN = [
  ...
  /* O terceiro elemento perdoa o padrão quando o contexto anterior já explica
     que o valor é o preço da nossa assinatura. A regra existe para impedir que
     uma matéria invente o salário de um cargo, não para impedir o site de
     dizer quanto cobra. O perdão é estreito de propósito: só as palavras da
     assinatura valem, porque "paga" ou "plano" apareceriam também em
     "o cargo paga R$ 8 mil por mês", que é exatamente o que não pode passar. */
  [/R\$\s?\d[\d.,]*\s*(mil|por m[êe]s|mensa)/i, 'informa valor de remuneração', /\bassin(atura|ante|ar)\b|\bmensalidade\b/i],
...
function factualProblems(text) {
  const problems = [];
  for (const [pattern, label, perdao] of FORBIDDEN) {
    ...
    if (perdao && perdao.test(text.slice(Math.max(0, m.index - 160), m.index))) continue;
