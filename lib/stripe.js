const { getProduct, getSellableProduct } = require('./catalog');
...
async function verifyPurchase(sessionId, expectedSlug) {
  ...
  if (session.metadata?.site_id !== 'concurso_audiobook' || session.metadata?.project_id !== 'concurso_audiobook') return null;
...
async function findPaidPurchasesByEmail(email) {
  ...
  for (const customer of customers.data || []) {
    ...
    for (const session of sessions.data || []) {
      if (session.metadata?.site_id !== 'concurso_audiobook' || session.metadata?.project_id !== 'concurso_audiobook') continue;
      const slug = sessionProduct(session);
