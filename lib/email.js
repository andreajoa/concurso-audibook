...
async function sendWithResend({ to, subject, html, text, scheduledAt, tags, idempotencyKey, replyTo }) {
...
async function sendWithSmtp({ to, subject, html, replyTo }) {
  ...
  return transporter.sendMail({from:fromAddress(),to,subject,html,replyTo});
}
...
function accessEmailHtml({ productName, accessUrl }) {
  ...
  return shell({eyebrow:'COMPRA APROVADA',title:'Sua trilha está liberada.',body,ctaLabel:'Abrir meu material',ctaUrl:accessUrl,footer:'<p style="color:#405047">Estude com direção, revise com constância e use o material para identificar onde ainda precisa reforçar.<br><strong>Trilha Aprova</strong></p>'});
}

async function sendAccessEmail({to,productName,accessUrl,idempotencyKey}) {
  return deliver({to,subject:`Trilha Aprova — seu acesso a ${productName}`,html:accessEmailHtml({productName,accessUrl}),tags:[{name:'type',value:'purchase'}],idempotencyKey});
}

/* A chave do caderno é a identidade da pessoa, e este e-mail é o único lugar
   onde ela existe: o site não tem senha e o banco guarda só o valor. Por isso
   o texto pede para guardar antes de pedir qualquer outra coisa. */
async function sendCadernoKeyEmail({to,chave,accessUrl}) {
  const body=`<p>Sua assinatura do <strong>caderno de erros</strong> está ativa. A partir de agora o que você anota no celular aparece no computador, e a fila de revisão continua de onde parou.</p><p>Abra o link abaixo uma vez em cada aparelho que você usa para estudar. Ele guarda a chave ali e não pergunta mais nada — não há senha para esquecer.</p><div style="padding:17px;border-radius:15px;background:#fbf8f1;border-left:4px solid #caa65a"><strong style="color:#08284f">Guarde este e-mail.</strong><p style="margin:6px 0 0;color:#697586">Esta chave é o seu acesso e não pode ser recuperada em nenhuma outra tela. Se apagar o e-mail, escreva a chave em outro lugar: <code style="font-family:monospace;word-break:break-all">${escapeHtml(chave)}</code></p></div>`;
  return deliver({to,subject:'Trilha Aprova — a chave do seu caderno de erros',html:shell({eyebrow:'ASSINATURA ATIVA',title:'Seu caderno agora acompanha você.',body,ctaLabel:'Abrir meu caderno',ctaUrl:accessUrl}),tags:[{name:'type',value:'subscription'}]});
}
...
async function sendPaymentFailedEmail({to,productName,returnUrl}) {
  const body=`<p>A tentativa de pagamento de <strong>${escapeHtml(productName)}</strong> não foi confirmada.</p><p>Isso pode acontecer por limite, validação do banco ou uma interrupção temporária. Nenhuma nova cobrança será feita por este e-mail.</p>`;
...
async function sendContactEmail({name,phone,email,subject,message,requestId}) {
  const body=`<p><strong>Nome:</strong> ${escapeHtml(name)}</p><p><strong>Telefone:</strong> ${escapeHtml(phone)}</p><p><strong>E-mail:</strong> ${escapeHtml(email)}</p><p><strong>Assunto:</strong> ${escapeHtml(subject)}</p><hr><p style="white-space:pre-wrap">${escapeHtml(message)}</p>`;
  return deliver({to:'suporte@concursotrilhaaprova.online',replyTo:email,subject:`[Contato Trilha Aprova] ${subject}`,html:shell({eyebrow:'FORMULÁRIO DE CONTATO',title:'Uma nova mensagem chegou.',body}),idempotencyKey:`contact-${requestId}`,tags:[{name:'type',value:'contact'}]});
}

module.exports={sendContactEmail,sendAccessEmail,sendCadernoKeyEmail,sendAbandonedCheckoutEmail,sendPaymentFailedEmail,sendMarketingEmail,unsubscribeToken,unsubscribeUrl};
